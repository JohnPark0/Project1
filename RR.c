...
#define TIME_TICK 100000// 0.1 second
#define TIME_QUANTUM 3// 0.3 seconds
...
int main(int argc, char* argv[]) {
	...
	printf("[Log] processes are initialized.\n");
	//////////////////////////////////////////////////////////////////////////////////////////////////
	int ppid = getpid();
	...
	for (int outerLoopIndex = 0; outerLoopIndex < MAX_PROCESS; outerLoopIndex++) {
		...
		// child process
			...
			while (true) {
				// cpu burst part
				cpuBurstTime--;
				printf("[Log] child process(%d) cpu burst time %d.\n", getpid(), cpuBurstTime);

				if (cpuBurstTime == 0) {
					printf("[Log] sigusr 2 called.\n");
					cpuBurstTime = originCpuBurstTime[procNum];
					cmsgSnd(KEY[procNum], cpuBurstTime, ioBurstTime);
					kill(ppid, SIGUSR2);
				}
				else {
					printf("[Log] sigusr 1 called.\n");
					kill(ppid, SIGUSR1);
				}
				kill(getpid(), SIGSTOP);
			}
	...
	printf("[Log] processes are killed.\n");
	return 0;
}
